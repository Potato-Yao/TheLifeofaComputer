...
【JSON 严格输出格式】
你必须且只能输出一个合法的 JSON 对象，不要输出任何 Markdown 标记（如 ```json ）。
⚠️ 极度重要警告：stat_changes 中的正数绝对不能带 "+" 号（例如只能写 15，不能写 +15），否则会导致游戏崩溃！
{
      ...
      "outcomes": [
        {
          "probability": 1.0,
          "result_text": "结果描述",
          "stat_changes": {"health_hardware": 0, "health_system": 15, "mental_state": -5}
        }
      ]
    }
  ]
